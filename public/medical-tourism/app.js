...
function calculateROI() {
  const investment = Number.parseFloat(document.getElementById('initialInvestment').value)
  const returnRate = Number.parseFloat(document.getElementById('annualReturn').value) / 100
  const years = Number.parseFloat(document.getElementById('investmentYears').value)
  const costs = Number.parseFloat(document.getElementById('operatingCosts').value)

  // Update display values
  document.getElementById('investmentValue').textContent = `$${investment}M`
  document.getElementById('returnValue').textContent = `${Number.parseFloat(document.getElementById('annualReturn').value)}%`
  ...
  keyRisks.forEach((riskItem) => {
    ...
    card.innerHTML = `
            ...
            <div class="card__body">
                ...
                <p style="margin: 8px 0;">${riskItem.mitigation}</p>
            </div>
        `
...
function initializeImageLightbox() {
  // Inject minimal styles for the lightbox
  const style = document.createElement('style')
  style.textContent = `
    #imageLightboxOverlay { position: fixed; inset: 0; background: rgba(0,0,0,0.8); display:flex;align-items:center;justify-content:center;z-index:9999;opacity:0;visibility:hidden;transition:opacity .18s ease; }
    #imageLightboxOverlay.show { opacity: 1; visibility: visible; }
    #imageLightboxOverlay img { max-width: 90%; max-height: 92%; border-radius: 6px; box-shadow: 0 10px 30px rgba(0,0,0,0.6); padding: 2rem }
    #imageLightboxOverlay .caption { margin-top: 12px; margin-left: 12px; color: #fff; font-size: 0.95rem; text-align:center; }
    #imageLightboxClose { position: absolute; top: 18px; right: 18px; background: rgba(0,0,0,0.5); color: #fff; border: none; padding: 8px 10px; border-radius: 6px; cursor: pointer; font-size: 16px; }
    .responsive-image { cursor: zoom-in; }
    `
  ...
  function hideLightbox() {
    ...
    // small delay to allow transition before clearing src
    setTimeout(() => {
      img.src = ''
      caption.textContent = ''
  ...
  // Attach click listeners to images with class 'responsive-image'
  ...
  images.forEach((image) => {
    ...
    image.addEventListener('click', (e) => {
      ...
      // Use higher-resolution source if data-large provided
      const large = image.getAttribute('data-large')
      showLightbox(large || src, alt)
    })
  })
}
