...
<template>
  <!-- Profile Card -->
  <div
    class="nav-profile-card"
    :class="shrink ? 'nav-profile-card-shrink' : ''"
  >
    <!-- Avatar -->
    <ImageView
      :src="props.profileData.profilePictureUrl"
      :alt="props.profileData.name"
      class="img-pfp"
    />

    <!-- Name -->
    <h4 class="nav-profile-card-title lead-2">
      {{ props.profileData.name }}
    </h4>

    <!-- Role -->
    <h6 class="nav-profile-card-subtitle text-4 mb-1">
      {{ props.profileData.locales.role }}
    </h6>
  </div>
</template>
